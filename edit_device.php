...
if (!$device) {
    redirect('devices.php?message=Device tidak ditemukan');
}

$pageTitle = 'Edit Device';
$pageSubtitle = 'Perbarui informasi perangkat tanpa mengubah riwayat monitoring.';
$errors = [];
$values = $device;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            update_device($id, $values['nama_device'], $values['ip_address'], $values['jenis_device'], $values['lokasi']);
            redirect('devices.php?message=Device berhasil diperbarui');
